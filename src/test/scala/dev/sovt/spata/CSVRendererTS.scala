...
import dev.sovt.spata.CSVConfig.EscapeAll
import dev.sovt.spata.CSVConfig.EscapeMode
import dev.sovt.spata.CSVConfig.EscapeRequired
import dev.sovt.spata.CSVConfig.EscapeSpaces
      ...
      forAll(headerModes): headerMode =>
        forAll(escapeModes): escapeMode =>
          ...
          forAll(relevantCases): (_: String, headerCase: String, contentCase: String, _: List[String]) =>
            ...
            val stream = Stream(recs*).covaryAll[IO, Record]
            val out = stream
              .through(renderS(renderer, hdr, headerMode))
